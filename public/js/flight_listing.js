...
if (flightType == "one-way") {
    ...
    if (transfer) {
        ...
        axios
            .get(`${baseURL}/transfer/flight/${originAirportId}/${destinationAirportId}`)
            .then((response) => {
                console.log(response.data)

                loadFlights(response.data, "1", response.data[0].transferAirportID)
            })
            .catch((err) => {
                console.log(err)
            })
...
var loadFlights = async (db_response, numbering, transfer=null) => {
    ...
    if (transfer) {
        ...
        if (numbering == "1") {
            ...
            var transferAirport = await getAirportById(transfer)
        ...
        if (db_response.Message == "No transfer flights found") {
            $("#flight_list").append(`
        ...
        <div class="row mt-2">
            <h2>
                ${numbering}. ${originAirport[0].name} - ${transferAirport[0].name} - ${destinationAirport[0].name}
            </h2>
        </div>
        `)
